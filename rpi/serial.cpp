...
// its a bit cringe to have glocals in cpp file which is meant to be included but whatever
int serial_port;
...
int initializeSerial() {
    ...
    tty.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // Set non-canonical mode (raw mode)
...
/// Reads data sent from Arduino and prints it to stdout.
int readArduinoResponse() {
    char buf[256];
    ...
    if (n < 0) {
        ...
        std::cout << "Arduino responded: " << buf;
    }
    return 0;
}
